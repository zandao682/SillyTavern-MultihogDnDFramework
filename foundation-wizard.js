...
- Skill model (how skills/abilities work — omit for the default class skill tree): "SKILL_MODEL": { "groups": [ … ] }. Each group has { "id", "label", "model", "config" }. MULTIPLE groups may coexist. Models:
  * "skilltree" — the default prereq-DAG constellation tied to classes (config: {}).
  * "capabilities" — category-based abilities with per-category, formula-driven progression (Veridia-style). config: { "categories":["boon","title","skill"], "exclusive_category":"title", "category_progression":{"skill":"pp"}, "progressions":[ {"id":"none","type":"none"}, {"id":"pp","type":"points_tiers","levels_per_tier":10,"cost_formula":"100 * tier_rank","score_formula":"10 + total_levels * 2.5","points_label":"PP"} ] }. Progression types: none | counter | use_tracked | milestone | points_tiers | xp_levels. Formulas are arithmetic over tier_rank, skill_level, total_levels. A category mapped to "none" is a static ability that never levels; one mapped to a progressing profile levels up. This lets skills that level on use coexist with capabilities that don't.
  * "leveling" — simple skills that gain a numeric level with use/xp. config: { "type":"use_tracked", "threshold":100, "points_label":"XP" }.`;
